...
class MultiStockTradingEnv(gym.Env):
    ...
    def __init__(self, env_config) -> None:
        ...
        assert "df" in env_config and len(env_config["df"]) > 0, \
        ...
        self.change_matrix = self.change_matrix.to_numpy()\
            .reshape(-1, self.num_tickers) / 100.   # Percentage to decimal.
        self.feature_matrix = self.feature_matrix.to_numpy()\
            .reshape(-1, self.num_tickers, self.feature_dims)
    ...
    @property
    def state(self) -> None:
        return np.concatenate([self.feature_matrix[self.day, :, :].flatten(), self.portfolio], axis=0)
